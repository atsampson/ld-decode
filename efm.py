...
class EFM_PLL:
    ...
    def process(self, inputBuffer):
        ...
        for vCurr in inputBuffer:
            ...
            # Have we seen a zero-crossing, in the opposite direction to the
            # last one we saw?
            if ((vPrev < 0 and vCurr >= 0 and (not self.prevDirection))
                or (vPrev > 0 and vCurr <= 0 and self.prevDirection)):

                # Flip prevDirection
                self.prevDirection = not self.prevDirection
